...
def detect_orientation(image_array: np.ndarray) -> str:
    ...
    Returns:
        A string indicating the orientation (e.g., "a1", "h8"), or an empty string if not detected.
    """
    ...
    for bbox, text, _prob in results:
        ...
        if cleaned_text == "1" or cleaned_text == "8":
            numbers.append(
                {"text": cleaned_text, "center_x": center_x, "center_y": center_y}
            )
...
if __name__ == "__main__":
    """Demonstrates the usage of the detect_orientation function with a dummy image."""
    # Example Usage:
    ...
    cv2.putText(
        dummy_image, "h", (50, 350), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2
    )
    cv2.putText(
        dummy_image, "8", (100, 350), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2
    )
