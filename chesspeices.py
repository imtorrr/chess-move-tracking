...
class ChessPieces:
    ...
    def board_to_fen(self, board_idx, chessboard_obj):
        ...
        for r_idx in range(8): # Iterate through the board_idx array's rows (0-7)
            for c_idx in range(8): # Iterate through the board_idx array's columns (0-7)
                ...
                if piece_id != -1:
                    ...
                    fen_char = piece_id_to_fen_char.get(piece_name, 'U') # 'U' for unknown
                    board_chars[fen_rank_idx][fen_file_idx] = fen_char
                else:
                    board_chars[fen_rank_idx][fen_file_idx] = '1' # Placeholder for empty square
